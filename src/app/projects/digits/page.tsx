...
export default function TrainingCanvas() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      ...
      ></canvas>
      <div className="mt-2">
        <button onClick={clearCanvas} className="p-2 bg-gray-300 rounded mr-2">
          Clear
        </button>
        <button onClick={submitCanvas} className="p-2 bg-yellow-500 text-white rounded">
          Submit
        </button>
      </div>
      {isTraining == false && (
        <button onClick={trainModel} className="mt-4 p-2 bg-green-500 text-white rounded">
          Train Model
        </button>
      )}
      {isTraining == true && (
        <button onClick={trainModel} className="mt-4 p-2 bg-black text-white rounded">
          Currently Training
        </button>
      )}
      {isTraining == false && (
        <button onClick={testModel} className="mt-4 p-2 bg-blue-500 text-white rounded">
          Test Model
        </button>
      )}
