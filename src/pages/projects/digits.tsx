...
export default function TrainingCanvas() {
  ...
  const submitCanvas = () => {
    ...
    setTrainingData((prevData) => [
      ...prevData,
      { number: currentNumber, pixels: [...pixelArray.current] },
    ]);
  ...
  const testModel = async () => {
    ...
    const prediction = modelRef.current.predict(
      tf.tensor2d([testPixels], [1, 1024], "float32")
    ) as tf.Tensor;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      ...
      {predictedNumber !== null && (
        <h2 className="text-3xl font-bold text-red-500">{predictedNumber}</h2>
      )}
      {accuracy !== null && (
      ...
      <h1 className="text-1xl font-bold text-black mt-4">
